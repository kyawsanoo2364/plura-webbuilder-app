...
import { z } from "zod";
import { CreatePipelineFormSchema } from "@/components/form/createpipeline-form";
...
export const verifyAndAcceptInvitation = async () => {
  ...
  if (invatationExists) {
    const userDetails = await createTeamUser(invatationExists.agencyId, {
      email: invatationExists.email,
      agencyId: invatationExists.agencyId,
      avatarUrl: user.imageUrl,
      id: user.id,
      name: `${user.firstName} ${user.lastName}`,
      role: invatationExists.role,
      createdAt: new Date(),
      updatedAt: new Date(),
    });
...
export const changeUserPermission = async (
  ...
  try {
    const response = await db.permissions.upsert({
      ...
      create: { access: permission, email: email, subAccountId },
    });
    return response;
  } catch (error) {
    console.log(error);
...
export const getSubaccountDetails = async (subaccId: string) => {
  const response = await db.subAccount.findUnique({ where: { id: subaccId } });
  return response;
};

export const deleteSubaccount = async (subaccId: string) => {
  const response = await db.subAccount.delete({ where: { id: subaccId } });
  return response;
};
...
export const sendInvitation = async (
  ...
  try {
    ...
    // If an invitation already exists, revoke it before sending a new one
    if (existingInvitation) {
      await client.invitations.revokeInvitation(existingInvitation.id);
      console.log(`Revoked previous invitation for ${email}`);
    }
    ...
    const invitationPayload = {
      ...
      publicMetadata: {
        role,
        throughInviation: true,
      },
    };

    await client.invitations.createInvitation(invitationPayload);
  } catch (error) {
...
export const upsertContact = async (
  ...
  try {
    if (!contact) {
      throw new Error("Contact is required and cannot be null or undefined");
    }
...
export const getDomainContent = async (subDomainName: string) => {
  const response = db.funnel.findUnique({
    where: { subDomainName },
    include: { FunnelPages: true },
  });
